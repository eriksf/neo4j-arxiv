...
Should produce a file name `arxiv-metadata-oai-snapshot.json`. It contains a JSON entry per line and
has about 1.8 million records.

## Usage

1. Copy `.env.sample` to `.env` and fill in the variables.

2. Run the `insert.py` script to insert data. Specify the input file (JSON file from above), start line to
